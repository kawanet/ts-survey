...
describe("runFix --semicolons off (dry-run, sample/semicolons-mixed)", () => {
    it("strips every trailing `;` from ASI-eligible statements in-memory", async () => {
        const project = new Project({tsConfigFilePath: SAMPLE_TSCONFIG})
        await runFix(project, {...SEMI_OFF, report: {}, semicolons: "off"})
...
describe("runFix --semicolons on (dry-run, sample/semicolons-mixed)", () => {
    it("appends `;` to every ASI-eligible statement lacking one", async () => {
        const project = new Project({tsConfigFilePath: SAMPLE_TSCONFIG})
        await runFix(project, {...SEMI_OFF, report: {}, semicolons: "on"})
...
describe("runFix --semicolons off handles nested ASI-eligible statements", () => {
...
describe("runFix --semicolons off keeps `;` at ASI-hazard sites", () => {
